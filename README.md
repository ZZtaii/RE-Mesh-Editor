...
For a direct hybrid export of a subset, enable **Selected Objects Only** and select the desired LOD0 mesh objects in Object Mode. The output contains only those parts; unselected parts and lower LODs are omitted. The armature does not need to be selected. Each selected part keeps only the source shape links that belong to it, with original deltas on verified vertices and any explicit Blender deltas on added vertices. Selecting only parts without source shape links is allowed and produces a mesh without blend shapes; the exporter reports this explicitly. A shaped replacement with neither safely mapped source vertices nor nonzero matching shape-key deltas is refused when selected alone. The Fluffy mod folder exporter continues to export the whole collection.

### Transfer SF6 body shape keys to edited geometry

Import the original body donor with **SF6: Preserve Source + Shape Keys**. In the 3D View sidebar, open **RE Mesh → SF6 Shape Transfer**, choose **Original Body** and **Edited Body**, then click **Preview**. The preview shows how many edited vertices can be matched, which keys correspond, and which vertices and keys will be left alone. **Leg Shapes Only** is enabled by default; it transfers matching knee/thigh corrections while leaving chest and arm corrections on the edited body unchanged. You can turn it off to include other matching body keys.

The tool matches body shape keys by the anatomical name after the `BS_BodyNN.` prefix, so equivalent knee keys can match across costumes with different Body numbers. It keeps all vertices verified against the edited body's embedded retail source unchanged. For other vertices it uses matching UV coordinates when available, then nearby donor triangles to interpolate relative shape movement. **Max Match Distance** limits the search. Existing nonzero edits on those vertices are kept, and keys without a matching donor are left alone. Preview changes nothing; **Transfer** is undoable. Geometry absent from the donor has no corrective movement to copy.

To write transferred movement into an SF6 mesh with added topology, export the edited collection with **SF6: Preserve Source Data** and **SF6: Hybrid Shape Export (LOD0)**. The report separates original source deltas from transferred deltas. Inspect the result in Blender and test it in-game; interpolation across different costumes can need manual cleanup at seams or joint bends.
